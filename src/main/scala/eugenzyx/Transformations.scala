...
trait Transformations {
  ...
  def responseToIdTextPairRDD: TweetSummary => (Long, String) = tweetSummary => tweetSummary match {
  ...
  def toTweetSummary: Row => TweetSummary = row => (row.getAs[String]("id"), row.getAs[String]("text"), row.getAs[String]("inReplyToStatusId"))

  def onlyValidRecords: TweetSummary => Boolean = tweetSummary => tweetSummary match {
